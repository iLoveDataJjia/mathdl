...
% Vanilla gradient descent convergence and rate
\begin{proposition}
  {\normalfont
...
\begin{proof}
  Let $f$ \textit{convex}, \textit{L-smooth}, admits a \textit{global minimum} $x^*$ and $\alpha < \frac{2}{L}$ and $n \in \mathbb{N}$.
  \begin{equation*}
    \begin{array}{ll}
      \norm{x^{(n+1)} - x^*}^2_m  & = \norm{x^{(n)} - x^* - \alpha {\nabla_f(x^{(n)})}}^2_m \\
  ...
  \begin{equation*}
    \begin{array}{llll}
      & \implies & (a) : & 0 \underset{(\ref{def:glob_min})}{\leq} f(x^{(n+1)}) - f(x^*) \leq f(x^{(n)}) - f(x^*) - \beta \norm{\nabla_f(x^{(n)})}^2
    \end{array}
  \end{equation*} \par
  Let $\forall n \in \mathbb{N}$, $\delta^{(n)} = f(x^{(n)}) - f(x^*)$.
...
\begin{definition}
  Let $\alpha \in \Real^{+*}$ and $f \in \mathcal{D}(\Real^m,\Real)$. Let the \textit{Stochastic gradient descent}
