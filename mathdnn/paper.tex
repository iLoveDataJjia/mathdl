...
\begin{abstract}
  ...
  important propositions and algorithms explaining dense neural network (DNN) theories. All the propositions will be mathematically proven as
...
\begin{proof}
  $\mathcal{N}_{c^+}$ is a composition of differentiable applications so it is differentiable by the \hyperref[theo:chain_rule]{chain rule} theorem.
...
\begin{definition}
  ...
  and will be noted as \begin{equation*}
    \mathcal{O}_{c^+}(((x^{(i)},{y^*}^{(i)}))_{i \in \llbracket 1,n \rrbracket},\cdot,\cdot)
  \end{equation*}
\end{definition}
...
\begin{corollary}
  {\normalfont
    $\mathcal{O}_{c^+}(((x^{(i)},{y^*}^{(i)}))_{i \in \llbracket 1,n \rrbracket},\cdot,\cdot) \in \mathcal{D}((\underset{k=1}{\overset{p}{\times}} \mathcal{M}_{m_{k},m_{k-1}} ) \times ( \underset{k=1}{\overset{p}{\times}} \Real^{m_{k}} ), \Real^{m_p})$. \par
...
\begin{proof}
  $\mathcal{O}_{c^+}(((x^{(i)},{y^*}^{(i)}))_{i \in \llbracket 1,n \rrbracket},\cdot,\cdot)$ is a sum and composition of differentiable applications so it is differentiable by the proposition $\ref{prop:jacobians_sum}$ and \hyperref[theo:chain_rule]{chain rule} theorem. \par
\end{proof}
