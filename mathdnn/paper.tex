...
\begin{definition}
  ...
  \textbf{Note:} For the multi-class classification problem $(\mathcal{P}_{c^+})$,
  $\mathcal{O}_{c^+}(X,Y^*,\cdot,\cdot)$ with all parameters fixed but one $W^{(k)}$ or $b^{(k)}$ will be $f$
  and $W^{(k)}$ or $b^{(k)}$ will be the numerical sequence $(x_v^{(n)})_{n \in \mathbb{N}}$.
