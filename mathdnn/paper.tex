...
\begin{abstract}
  ...
  important propositions explaining dense neural network (DNN) theories. All the propositions will be mathematically proven as
  much as possible and under assumptions if necessary. The subject used as reference is a multi-class classification problem with
  – dense layers, activation layers, Categorical cross-entropy loss and Stochastic gradient descent optimizer with decay and momentum. But all the
  elements below can be easily re-used or re-defined to cover regressions.
  \keywords{Dense neural network, Equation, Proof, Differentiability, Optimization}
\end{abstract}
...
% Subset convex definition
\begin{definition}
  Let $E \subseteq \Real^m$. $E$ \textit{convex} means \begin{equation*}
    \begin{gathered}
      \forall x \in E, \forall y \in E, \\
      \forall t \in [0,1], t * x + (1 - t) * y \in E
    \end{gathered}
  \end{equation*}
\end{definition}
...
% Applicaton convex
\begin{definition}
  Let $E \subseteq \Real^m$ \textit{convex} and \textit{open}, $F \subseteq \Real$ and $f \in \mathcal{D}(E,F)$. $f$ \textit{convex} means \begin{equation}\label{def:convex_eq}
    \begin{gathered}
      \forall x \in E, \forall y \in E, \\
...
\begin{proposition}
  {\normalfont
    Let $E \subseteq \Real^m$ \textit{convex} and \textit{open}, $F \subseteq \Real$ and $f \in \mathcal{D}(E,F)$. $f$ \textit{convex} is equivalent to \begin{equation}\label{prop:convex_grad}
      \begin{gathered}
        \forall x \in E, \forall y \in E, \\
...
\begin{proof}
  Suppose $f$ \textit{convex} $(\ref{def:convex_eq})$. Let $x \in E$ and $y \in E$. \begin{equation*}
    \begin{gathered}
      \forall t \in ]0,1[,
      f(x + t * (y - x)) \underset{\norm{\cdot}_m \in \mathcal{C}(\Real^m,\Real),f \in \mathcal{D}(E,F),(\ref{def:differentiable})}{=}
        f(x) + \frac{\partial f}{\partial (t * (y - x))}(x) + \underset{t \to 0}o(\norm{t * (y - x)}_m)
    \end{gathered}
  \end{equation*}
  \begin{equation*}
    \begin{array}{ll}
      ...
      & \underset{\frac{\partial f}{\partial \cdot} \in \mathcal{L}(\Real^m,\Real)}{\implies}
        \forall t \in ]0,1[,
        \frac{\partial f}{\partial (y - x)}(x) + \underset{t \to 0}o(\norm{y - x}_m)
        \leq f(y) - f(x) \\
      & \underset{t \to 0}{\implies}
  ...
  Suppose $(\ref{prop:convex_grad})$. Let $x \in E$, $y \in E$ and $t \in [0,1]$. Let $z = t * x + (1 - t) * y$.
  \begin{equation*}
    \begin{array}{lll}
      f(z) - (1 - t) * \nabla_f(z) * (y - x)^T & \underset{(\ref{prop:convex_grad})}{=} f(z) + \nabla_f(z) * (x - z)^T \leq f(x) & (a) \\
      f(y) + t * \nabla_f(z) * (y - x)^T & \underset{(\ref{prop:convex_grad})}{=} f(z) + \nabla_f(z) * (y - z)^T \leq f(y) & (b)
    \end{array}
  \end{equation*}
  \begin{equation*}
    \begin{array}{ll}
      & \underset{t * (a) + (1 - t) * (b)}{\implies} f(z) \leq t * f(x) + (1 - t) f(y)
    \end{array}
  \end{equation*}
\end{proof}
...
% L-smooth definition
\begin{definition}
  Let $E \subseteq \Real^m$, $F \subseteq \Real$, $f \in \mathcal{D}(E,F)$ and $L \in \Real^{+*}$. $f$ \textit{L-smooth} means \begin{equation*}
    \begin{gathered}
      \forall x \in E, \forall y \in E, \\
      \norm{\nabla_f(x) - \nabla_f(y)}_m \leq L * \norm{x - y}_m 
    \end{gathered}
