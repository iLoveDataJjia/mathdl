...
\begin{abstract}
  ...
  as reference is a multi-class classification problem with – dense layers, andactivation layers, Categorical cross-entropy loss and Stochastic
...
% ReLU definition
\begin{definition}
  Let the application $ReLU$ noted as $\mathcal{R}$ be
  \begin{equation*}
    \begin{array}{llll}
...
% Softmax definition
\begin{definition}
  Let the application $Softmax$ noted as $\mathcal{S}$ be
  \begin{equation*}
    \begin{array}{llll}
      \mathcal{S} & : & \Real^m & \longrightarrow \Real^m \\
      &   & z & \longmapsto \begin{bmatrix}
...
\begin{proposition}
  {\normalfont
    $\mathcal{S} = (\mathcal{S}_1 \ldots \mathcal{S}_m) \in \mathcal{F}_{act}(\Real^m,\Real^m)$ and its jacobian is \begin{equation}
      \begin{array}{llll}
