...
%Kronecker delta
\begin{notation}
  The notation $\delta_{i,j}$ means the kronecker delta application \begin{equation*}
    \begin{array}{llll}
...
\begin{definition}
  Let $E \subseteq \Real^m$, $y^* \in \Real^m$ and $F_{loss}^{y^*} \in \mathcal{D}(E,\Real)$.
  $F_{loss}^{y^*}$ is a loss function is equivalent to \begin{equation*}
    \begin{array}{llll}
      F_{loss}^{y^*} \circ g & : & \{\epsilon \in \Real^m | y^* + \epsilon \in E\} & \longrightarrow \Real \\
      & & \epsilon & \longmapsto F_{loss}^{y^*}(y^* + \epsilon)
    \end{array}
  \end{equation*} \par
  is an inscreasing function according $\norm{\epsilon}_m$. \par
  $y^*$ is named the ground truth matrix. \par
  The notation $\mathcal{F}_{loss}(E)$ means the set of loss functions from $E$ to $\Real$.
\end{definition}

% Categorical cross-entropy loss definition
\begin{definition}
  Let $y^* \in \{0,1\}^m$. Let the application \textit{Categorical cross-entropy} noted as $\xi^{y^*}$ be
  \begin{equation*}
    \begin{array}{llll}
      \xi^{y^*} & : & ]0,1[^m & \longrightarrow \Real^m \\
      &   & y & \longmapsto - \sum_{k=1}^m y_k^* * \log(y_k)
    \end{array}
  \end{equation*} \par
\end{definition}
